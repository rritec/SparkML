...
object ProvSysClmAgg {
def main(args: Array[String]): Unit = {
try
  {
val spark = GlobalContext.createSparkSession("JSON Document Creation for ProviderSystem Level Claims Aggregation Collection")
LoggerApp.logger.info("JSON Documents creation  for ProviderSystem Level Claims Aggregation Started at : " + DateApp.getDateTime())
...
val mongocollection  = spark.sparkContext.getConf.get("spark.mongo.ProvSysClmAgg")
println("mongocollection :" + mongocollection)
val outputProvSysClmAgg = spark.sparkContext.getConf.get("spark.output.ProvSysClmAgg")
println("outputProvSysClmAgg :" + outputProvSysClmAgg)
println("ProvSysClmAgg - Table: " + System.currentTimeMillis())
...
val JSONDoc = FinalJoin.select(col("ProviderSystem"),col("ProviderKey"),col("ReportingPeriod"),col("EI"),col("CS"),col("MR"),col("ALL")).dropDuplicates()
JSONDoc.repartition(1).write.mode("overwrite").format("json").save("maprfs://".concat(outputProvSysClmAgg))
LoggerApp.logger.info("JSON Documents creation  for ProviderSystem Level Claims Aggregation Ended at : " + DateApp.getDateTime())
LoggerApp.logger.info("Loading ProviderSystem Level Claims Aggregation data into Mongo Collection:ProvSysClmAgg Started at : " + DateApp.getDateTime())
val loc = "maprfs://".concat(outputProvSysClmAgg)
SparkMongoLoad.MongoLoad(loc, mongodbname, mongocollection, mongohost, mongoport, mongouname, mongopassword)
LoggerApp.logger.info("Loading ProviderSystem Level Claims Aggregation data into Mongo Collection:ProvSysClmAgg Ended at : " + DateApp.getDateTime())
}
catch {
      case e: Exception => LoggerApp.logger.info(s"Exception while JSON Documents creation for ProviderSystem Level Claims Aggregation Collection" :+ e.printStackTrace())
    }
