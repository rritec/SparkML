...
try
  {
val spark = GlobalContext.createSparkSession("JSON Document Creation for Provider-TIN Level Claims Summary Collection")
  
LoggerApp.logger.info("JSON Documents creation  for Provider-TIN Level Claims Summary Started at : " + DateApp.getDateTime())
...
val mongocollection  = spark.sparkContext.getConf.get("spark.mongo.ProvTINClmSumm")
...
val outputProvTINClmSumm = spark.sparkContext.getConf.get("spark.output.ProvTINClmSumm")
println("outputProvTINClmSumm :" + outputProvTINClmSumm)
println("ProvTINClmSumm - Table: " + System.currentTimeMillis())
...
val df1 = spark.sql(s"""select  providersyskey,case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end as ReportingPeriod,HealthcareOrganization as ProviderSystem,BusinessSegmentIdentifier,formattedtin as TIN,ClaimDenialCategoryLevel1ShortName,sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as DenialCount,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END) as DenialAmount from source where PaidOrDeniedCode = 'D' group by case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end,HealthcareOrganization,formattedtin,BusinessSegmentIdentifier,ClaimDenialCategoryLevel1ShortName,providersyskey""")
df1.createOrReplaceTempView("df1")
val dfGrp1 = spark.sql("select * from df1 ")
val dfGrp2 = spark.sql(s"""select providersyskey,case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end as ReportingPeriod,HealthcareOrganization as ProviderSystem,BusinessSegmentIdentifier,formattedtin as TIN,sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as ClaimsDenied,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END) as AmountDenied,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN masteridclaimcount ELSE 0 END) as ClaimsPaid,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountPaid,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN masteridclaimcount ELSE 0 END)+sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as ClaimsSubmitted,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END)+Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountSubmitted,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN masteridclaimcount ELSE 0 END)+sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as ClaimsCompleted,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END)+Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountCompleted,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountUHCPaid,Sum(case when FinalPassIndicator = 'Y' THEN BilledAmount ELSE 0 END) as AmountBilled,Sum(ActualAllowedAmount) as AmountActualAllowed,Sum(ActualAllowedAmount)+sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END) as AmountExpectedAllowed from source group by case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end,HealthcareOrganization,formattedtin,BusinessSegmentIdentifier,providersyskey""")

val df3 = spark.sql(s"""select  providersyskey,case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end as ReportingPeriod,HealthcareOrganization as ProviderSystem,formattedtin as TIN,ClaimDenialCategoryLevel1ShortName,sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as DenialCount,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END) as DenialAmount from source where PaidOrDeniedCode = 'D' group by case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end,HealthcareOrganization,formattedtin,ClaimDenialCategoryLevel1ShortName,providersyskey""")
df3.createOrReplaceTempView("df3")
val dfGrp3 = spark.sql("select * from df3 ")
val dfGrp4 = spark.sql(s"""select providersyskey,case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end as ReportingPeriod,HealthcareOrganization as ProviderSystem,formattedtin as TIN,sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as ClaimsDenied,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END) as AmountDenied,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN masteridclaimcount ELSE 0 END) as ClaimsPaid,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountPaid,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN masteridclaimcount ELSE 0 END)+sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as ClaimsSubmitted,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END)+Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountSubmitted,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN masteridclaimcount ELSE 0 END)+sum(case when PaidOrDeniedCode = 'D' THEN masteridclaimcount ELSE 0 END) as ClaimsCompleted,sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END)+Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountCompleted,Sum(case when PaidOrDeniedCode = 'P' and FinalPassIndicator = 'Y' THEN UHCPaidAmount ELSE 0 END) as AmountUHCPaid,Sum(case when FinalPassIndicator = 'Y' THEN BilledAmount ELSE 0 END) as AmountBilled,Sum(ActualAllowedAmount) as AmountActualAllowed,Sum(ActualAllowedAmount)+sum(case when PaidOrDeniedCode = 'D' THEN EstimatedAllowedSelectDeniedAmount ELSE 0 END) as AmountExpectedAllowed from source group by case when month(IncurredServiceDate) < 10 then concat(0,month(IncurredServiceDate),year(IncurredServiceDate)) when month(IncurredServiceDate) >= 10 then concat(month(IncurredServiceDate),year(IncurredServiceDate)) else 0 end,HealthcareOrganization,formattedtin,providersyskey""")
...
val JSONDoc = RPFinal.select(col("ProviderSystem"),col("ProviderKey"),col("TIN"),col("ReportingPeriod"),col("EI"),col("CS"),col("MR"),col("ALL")).dropDuplicates()
JSONDoc.repartition(1).write.mode("overwrite").format("json").save("maprfs://".concat(outputProvTINClmSumm))

LoggerApp.logger.info("JSON Documents creation  for Provider-TIN Level Claims Summary Ended at : " + DateApp.getDateTime())
LoggerApp.logger.info("Loading Provider-TIN Level Claims Summary data into Mongo Collection:ProvTINClmSumm Started at : " + DateApp.getDateTime())
val loc = "maprfs://".concat(outputProvTINClmSumm)
SparkMongoLoad.MongoLoad(loc, mongodbname, mongocollection, mongohost, mongoport, mongouname, mongopassword)
LoggerApp.logger.info("Loading Provider-TIN Level Claims Summary data into Mongo Collection:ProvTINClmSumm Ended at : " + DateApp.getDateTime())
  }
catch {
      case e: Exception => LoggerApp.logger.info(s"Exception while JSON Documents creation for Provider-TIN Level Claims Summary Collection" :+ e.printStackTrace())
    }
}
}